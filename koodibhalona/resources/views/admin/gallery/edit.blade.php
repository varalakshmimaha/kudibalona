...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        <form action="{{ route('admin.gallery.update', $item->id) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-8">
            @csrf
            @method('PUT')
            
            <!-- Current Image -->
            <div class="space-y-4">
                <label class="block text-sm font-bold text-gray-700">Change Image</label>
                <div class="relative group">
                    <input type="file" name="image" id="image-input"
                        class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10">
                    <div class="border-2 border-dashed border-gray-200 rounded-2xl p-6 text-center group-hover:border-amber-400 transition-colors bg-gray-50">
                        <div id="preview-container" class="mb-4">
                            <img id="image-preview" src="{{ asset('storage/' . $item->image) }}" alt="Preview" class="max-h-48 mx-auto rounded-xl">
                        </div>
                        <div id="upload-placeholder" class="hidden">
                            ...
                            <p class="text-sm text-gray-500 font-medium">Click to change image</p>
                        </div>
            ...
            <div class="grid grid-cols-2 gap-8 pt-4">
                ...
                <div class="space-y-4">
                    ...
                    <label class="flex items-center cursor-pointer pt-3">
                        <div class="relative">
                            <input type="checkbox" name="is_active" value="1" class="sr-only" {{ $item->is_active ? 'checked' : '' }}>
            ...
            <div class="space-y-4 pt-4">
                <label class="flex items-center cursor-pointer">
                    <input type="checkbox" name="is_large" value="1" class="w-5 h-5 rounded border-gray-300 text-amber-500 focus:ring-amber-500" {{ $item->is_large ? 'checked' : '' }}>
